...
def api_filter():
    if 'key' in request.args:
        if str(request.args['key']) == api_key:
            ...
            output = []
            for s in mongo.db.cellData.find(q).limit(limit):
                output.append({'time Stamp' : s['timeStamp'],'tower' : s['tower'],'telephone' : s['teNumber'], 'cellPhoneOS' : s['cellPhoneOS'], 'conQuality' : s['conQuality'], 'distFromToer' : s['distFromToer'], 'callTo' : s['callTo'], 'callDuration' : s['callDuration']})
                # print(output)
            return jsonify({'result' : output})
        else:
            return "Error: KEY is wrong / missing ."
